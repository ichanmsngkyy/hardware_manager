  ...
  function separateOrdersByStatus(filteredOrders) {
    ...
    const otherOrders = groupedOrders.filter(
      (group) => group.status !== "Pending"
    );
  ...
  function groupOrdersByOrderId(orders) {
    ...
    return Object.values(grouped).map(group => {
      group.items.sort((a, b) => a.partName.localeCompare(b.partName));
      return group;
    });
  }
  ...
  function createOrderRowFromTemplate(orderGroup) {
    const row = document.createElement("tr");
    const statusClass = orderGroup.status.toLowerCase().replace(/\s+/g, "-");
    const statusColor =
    ...
    const partsList = orderGroup.items.map(item => 
      `<div style="margin-bottom: 4px;">
        <span style="font-weight: 600;">${item.partName}</span>
        <span style="color: #666; margin-left: 8px;">(Qty: ${item.quantity})</span>
      </div>`
    ).join('');
    
    // Calculate total quantity
    const totalQuantity = orderGroup.items.reduce((sum, item) => sum + parseInt(item.quantity), 0);
    
    row.innerHTML = `
      <td><strong>${orderGroup.orderId}</strong></td>
      <td>
      ...
      <td style="font-weight: 600; font-size: 1.1rem;">${totalQuantity}</td>
      <td><span class="status-badge status-${statusClass}" style="background: ${statusColor}; color: white; padding: 4px 12px; border-radius: 12px; font-size: 0.9rem;">${orderGroup.status}</span></td>
      <td>
        <div class="actions">
          <button class="btn btn-warning btn-sm" onclick="editOrder('${orderGroup.orderId}')">✏️</button>
          <button class="btn btn-danger btn-sm" onclick="confirmDeleteOrder('${orderGroup.orderId}')">🗑️</button>
        </div>
      </td>
    `;
    return row;
...
async function fetchOrders() {
  ...
  orders = rawOrders.map((order) => ({
        ...
        categoryId: order.categoryId || order.category_id,
...
function getStatusColor(stockLevel) {
  switch (stockLevel) {
    ...
    case "Out of Stock": return "#e74c3c";
    case "Discontinued": return "#7f8c8d";
    default: return "#7f8c8d";
  }
}
...
function openAddPartModal() {
  ...
  // Add event listener for automatic category detection
  partNameInput.addEventListener("input", autoDetectStorageCategory);
  
  function autoDetectStorageCategory() {
    const partName = this.value.toLowerCase();
    const storageKeywords = [
      "ssd", "hdd", "nvme", "m.2", "sata", "storage", "hard drive", 
      "solid state", "flash", "disk", "drive", "sshd", "hybrid"
    ];
    
    const isStorage = storageKeywords.some(keyword => partName.includes(keyword));
    if (isStorage) {
      partCategory.value = "Storage";
      partCategory.dispatchEvent(new Event("change"));
    }
  }
...
function editPart(id) {
  const part = inventory.find(
    ...
    if (!part) return;
  document.getElementById("partModalTitle").textContent = "Edit Part";
  document.getElementById("partId").value = part.categoryId; // Use the categoryId instead of database ID
  document.getElementById("partId").readOnly = true;
  document.getElementById("partName").value = part.name;
  document.getElementById("partBrand").value = part.brand;
  
  // Add event listener for automatic category detection in edit mode
  const partNameInput = document.getElementById("partName");
  const partCategory = document.getElementById("partCategory");
  
  // Remove any existing listeners to avoid duplicates
  partNameInput.removeEventListener("input", autoDetectCategory);
  partNameInput.addEventListener("input", autoDetectCategory);
  ...
  // Check if the part name contains storage keywords and set category accordingly
  ...
  const isStorage = storageKeywords.some(keyword => partName.includes(keyword));
  if (isStorage && partCategory) {
    partCategory.value = "Storage";
  } else if (partCategory) {
    // Only set the original category if it's not a storage item
    let attempts = 0;
    const setCategory = () => {
      partCategory.value = part.category;
      if (partCategory.value !== part.category && attempts < 5) {
  ...
  const partModal = document.getElementById("partModal");
  if (partModal) partModal.style.display = "block";
  currentEditPartId = part.categoryId; // Use the categoryId for updates
}
...
async function handlePartFormSubmit(e) {
  ...
  const partStatusElement = document.getElementById("partStatus");
    ...
    // Prevent adding parts with zero or negative stock
    if (quantity <= 0) {
      showToast("Quantity must be greater than 0. Please enter a positive number.", "error");
      return null;
    }

    // Prevent negative prices
    if (price < 0) {
      showToast("Price cannot be negative. Please enter a positive number.", "error");
      return null;
    }

    // Prevent negative alert thresholds
    if (alertThreshold < 0) {
      showToast("Alert threshold cannot be negative. Please enter a positive number.", "error");
      return null;
    }
    ...
    if (existingPartByName) {
      showToast(`A part with the name "${name}" already exists. Please use a different name.`, "error");
      return null;
    }
    ...
    if (existingPartById) {
      showToast(`A part with the ID "${categoryId}" already exists. Please use a different ID.`, "error");
      return null;
    }
...
function renderOrdersTable(tableBody, pendingOrders, otherOrders) {
  ...
  if (pendingOrders.length === 0 && otherOrders.length === 0) {
    ...
    const groupedPendingOrders = groupOrdersByOrderId(pendingOrders);
    const groupedOtherOrders = groupOrdersByOrderId(otherOrders);

    // Add pending orders (grouped)
    groupedPendingOrders.forEach((orderGroup) => {
        const row = createOrderRowFromTemplate(orderGroup);
        tableBody.appendChild(row);
    });
    ...
    // Add other orders (grouped)
    groupedOtherOrders.forEach((orderGroup) => {
        const row = createOrderRowFromTemplate(orderGroup);
        tableBody.appendChild(row);
    });
}
...
function createOrderRowFromTemplate(orderGroup) {
  ...
  const trElement = row.querySelector(".order-row");
    ...
    const statusClass = orderGroup.status.toLowerCase();
  const isCancelled = orderGroup.status === "Cancelled";
  ...
  const isPending = orderGroup.status === "Pending";
    ...
    // Format date
  const formattedDate = new Date(orderGroup.date).toLocaleDateString("en-US", {
    year: "numeric",
    month: "short",
    day: "numeric",
    });
  ...
  const statusBadgeElement = row.querySelector(".status-badge");
    ...
    if (orderIdElement) {
      orderIdElement.textContent = orderGroup.orderId;
      // Store the order ID for editing (not individual order ID)
      orderIdElement.setAttribute('data-order-id', orderGroup.orderId);
    }
    
    // Display multiple parts in the part name column
    if (partNameElement) {
      if (orderGroup.items.length === 1) {
        partNameElement.textContent = orderGroup.items[0].partName;
      } else {
        // Show first part + count of additional parts
        const firstPart = orderGroup.items[0].partName;
        const additionalCount = orderGroup.items.length - 1;
        partNameElement.textContent = `${firstPart} +${additionalCount} more`;
      }
    }
    ...
    if (statusBadgeElement) {
        statusBadgeElement.textContent = orderGroup.status;
...
async function createNewOrders(formData) {
  ...
  // Always refresh the data after order creation attempt
  if (successCount > 0) {
    await refreshOrderData();
  }
}
  ...
  container.innerHTML = "";
    ...
    let categoryParts = inventory.filter(
      (part) => part.category === category.category
        ...
        if (currentEditOrderId) {
      const currentOrder = orders.find((o) => o.id === currentEditOrderId);
            if (currentOrder && currentOrder.categoryId === category.partId) {
        const currentPart = inventory.find(
          (p) => p.categoryId === currentOrder.categoryId
        );
        if (
          currentPart &&
          !categoryParts.find((p) => p.categoryId === currentPart.categoryId)
        ) {
    ...
    const partOptions = categoryParts
      .map((part) => {
        ...
        return `<option value="${part.categoryId}" ${
          part.categoryId === category.partId ? "selected" : ""
        } ${optionStyle}>
...
function updateCategoryPart(index, newPartId) {
    ...
    const part = inventory.find((p) => p.categoryId === newPartId);
        if (part) {
            // Calculate max stock for the new part
            let maxStock = part.quantity;
            if (currentEditOrderId) {
        const currentOrder = orders.find((o) => o.id === currentEditOrderId);
                if (currentOrder && currentOrder.categoryId === newPartId) {
  ...
  let pending = 0,
    ...
    orders.forEach((order) => {
      if (order.status === "Pending") pending++;
                ...
                completed++;
                // Find part price by categoryId (most reliable)
                let part = null;
                if (Array.isArray(inventory)) {
          part = inventory.find(
            (p) => p.categoryId && order.categoryId && p.categoryId === order.categoryId
          );
        }
        completedValue +=
...
function showOrderDetailsPopup(status) {
  ...
  const filtered = Array.isArray(orders)
    ...
    if (filtered.length === 0) {
        ...
        let html = `<table style="width:100%;border-collapse:collapse;"><thead><tr><th style='text-align:left;padding:8px;'>Order ID</th><th style='text-align:left;padding:8px;'>Part Name</th><th style='text-align:left;padding:8px;'>Date</th><th style='text-align:left;padding:8px;'>Quantity</th><th style='text-align:left;padding:8px;'>Unit Price</th><th style='text-align:left;padding:8px;'>Total Value</th><th style='text-align:left;padding:8px;'>Status</th></tr></thead><tbody>`;
    filtered.forEach((order) => {
      // Find the part to get its price from parts.price
      let part = null;
      let unitPrice = 0;
      if (Array.isArray(inventory)) {
        // Match by categoryId first (most reliable for exact matching)
        part = inventory.find(
          (p) => p.categoryId && order.categoryId === order.categoryId
        );
        
        // If not found by categoryId, try by partName (case-insensitive)
        if (!part) {
          part = inventory.find(
            (p) => p.name && order.partName && p.name.toLowerCase() === order.partName.toLowerCase()
          );
        }
        
        // If still not found, try by categoryId (this should be the primary method)
        if (!part) {
          part = inventory.find(
            (p) => p.categoryId && order.categoryId && p.categoryId === order.categoryId
          );
        ...
        // Get unit price from parts.price
        unitPrice = part.price ? parseFloat(part.price || 0) : 0;
        

      }
      
              const totalValue = unitPrice * parseInt(order.quantity || 0);
      const statusBadge = `<span style="background: #e8f5e9; color: #2e7d32; padding: 4px 8px; border-radius: 8px; font-size: 0.8rem;">${order.status}</span>`;
      ...
      html += `<tr><td style='padding:8px;'>${
        ...
        Number(unitPrice).toLocaleString("en-PH", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })
      }</td><td style='padding:8px;'>₱${
        Number(totalValue).toLocaleString("en-PH", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })
      }</td><td style='padding:8px;'>${statusBadge}</td></tr>`;
...
function renderDashboardWeeklyOrderChart() {
  ...
  async function populateOrderPartDropdown() {
    ...
    parts.forEach((part) => {
      const quantity = parseInt(part.quantity || 0);
      partDropdown.innerHTML += `<option value="${part.categoryId}">${part.name} (Available: ${quantity})</option>`;
    });
...
function handleEditClick(button) {
  ...
  // Get the order ID for editing (this is now the group order ID)
  ...
  // Call the async edit function with the order ID (handles multiple parts)
  editOrder(orderId);
}
...
// Fix the editOrder function to be async and handle inventory loading
async function editOrder(orderId) {
  ...
  // Pre-populate cart with all items from the order
  currentOrderCategories = [];
  
  for (const order of orderItems) {
    let part = inventory.find((p) => p.categoryId === order.categoryId);
    ...
    if (!part) {
      ...
      part = inventory.find((p) => p.categoryId === order.categoryId);
      if (!part) {
        showToast(`Could not load part information for ${order.partName}`, "error");
        continue;
      }
    }
  ...
  if (orderIdElement) orderIdElement.value = orderItems[0].orderId;
  if (orderDateElement) orderDateElement.value = orderItems[0].date;
  if (orderStatusElement) orderStatusElement.value = orderItems[0].status;
...
async function addInlineCategory() {
  ...
  // Check if part already exists in order - prevent duplicates
  const existingCategoryIndex = currentOrderCategories.findIndex(
    (cat) => cat.partId === partId
  );
  if (existingCategoryIndex !== -1) {
    showToast("This part is already in the order. Please adjust the quantity instead.", "error");
    return;
  }
