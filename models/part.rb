...
class Part
  ...
  def self.find_by_category_id(categoryId)
    results = query_sql("SELECT * FROM parts WHERE categoryId = '#{categoryId}'")
    return nil if results.empty?
  ...
  def self.update(id, attrs)
    ...
    current = query_sql("SELECT quantity, alertThreshold, name FROM parts WHERE id = #{id}").first
